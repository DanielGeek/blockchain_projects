...
impl Blockchain {
    ...
    fn starting_block(&mut self) {
        let genesis_block = Block {
            ...
            previous_hash: String::from("00000000000000000000"),
            nonce: 11316,
            hash: String::from("000015121212121212121212121"),
    ...
    fn is_block_valid(&self, new_block: &Block, lastest_block: &Block) -> bool {
        if new_block.previous_hash != lastest_block.hash {
            println!("Block with id {} has wrong previous hash", new_block.id);
            return false;
        } else if !new_block.hash.starts_with("0000") {
            println!("Block with id: {} has invidalid has", new_block.id);
            return false;
        } else if new_block.id != lastest_block.id +1 {
            println!("Block with id {} is not the next block after the latest block with id: {}", new_block.id, lastest_block.id);
            return false;
    ...
    fn chain_selector(&self, local: Vec<Block>, remote: Vec<Block>) -> Option<Vec<Block>> {
        ...
        match (is_local_valid, is_remote_valid) {
            ...
            (false, false) => {
                println!("Both the local and remote copies are not valid");
                None
            },
        }
    }
...
fn main(){
    ...
    let new_block = Block::new(3, new_BC.blocks[1].hash.to_owned(), "Dani".to_string());
    new_BC.try_add_block(new_block);

    let new_block = Block::new(4, new_BC.blocks[2].hash.to_owned(), "Angel".to_string());
    new_BC.try_add_block(new_block);
