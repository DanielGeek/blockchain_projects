...
contract ControlAcceso is AccessControl {
    ...
    modifier onlyWriter() {
        require(hasRole(rolWriter, msg.sender), "Only writter roles can execute the function");
        _;
    }

    modifier onlyAdmin() {
        require(hasRole(rolAdmin, msg.sender), "Only admin roles can execute the function");
        _;
    }
