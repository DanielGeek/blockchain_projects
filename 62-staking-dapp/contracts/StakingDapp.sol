...
error WithdrawAmountExceedTheBalance();
error LockIsActive();
...
contract StakingDapp is Ownable, ReentrancyGuard {
    ...
    function withdraw(uint _pid, uint _amount) public nonReentrant {
        ...
        if (user.amount < _amount) {
            revert WithdrawAmountExceedTheBalance();
        }
    ...
    function sweep(address token, uint256 amount) external onlyOwner {
        ...
        if (depositedTokens[token] < token_balance - amount  ) {
            revert CanNotWithdrawDepositedTokens();
        }
